...
import csv
import datetime
a = csv.reader(open('doc\Tor_15s_Layer2_85_MaxMin.csv', 'r'))
data = []
label = []
for row in a:
    data.append(row[:-1])
...
# print(len(dataset))
# print(len(label))
x_train, x_test, y_train, y_test = train_test_split(dataset, label, test_size=1/8., random_state=8)  # 分割训练集和测试集
estimators = {}
